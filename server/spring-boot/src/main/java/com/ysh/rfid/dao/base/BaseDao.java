...
public class BaseDao<T, P> {
     ...
     * @return 影响行数
     */
    protected Integer deleteById(String pk) {
        String tableName = getTableName();
        String sql = StrUtil.format("DELETE FROM {table} where uid = ?", Dict.create().set("table", tableName));
        log.debug("【执行SQL】SQL：{}", sql);
     ...
     * @return 操作的行数
     */
    protected Integer updateById(T t, String pk, Boolean ignoreNull) {
        ...
        String sql = StrUtil.format("UPDATE {table} SET {params} where uid = ?", Dict.create().set("table", tableName).set("params", params));
        log.debug("【执行SQL】SQL：{}", sql);
        log.debug("【执行SQL】参数：{}", JSONUtil.toJsonStr(values));
        return jdbcTemplate.update(sql, values);
    }
     ...
     * @return 单条记录
     */
    public T findOneById(String pk) {
        String tableName = getTableName();
        String sql = StrUtil.format("SELECT * FROM {table} where uid = ?", Dict.create().set("table", tableName));
     ...
     * @return 单条记录
     */
    public T findByName(String pk) {
        String tableName = getTableName();
        String sql = StrUtil.format("SELECT * FROM {table} where name = ?", Dict.create().set("table", tableName));
        RowMapper<T> rowMapper = new BeanPropertyRowMapper<>(clazz);
        log.debug("【执行SQL】SQL：{}", sql);
        log.debug("【执行SQL】参数：{}", JSONUtil.toJsonStr(pk));
        return jdbcTemplate.queryForObject(sql, new Object[]{pk}, rowMapper);
    ...
    public List<T> findByKind(T t,String kind){
        String tableName = getTableName(t);
        String sql = StrUtil.format("SELECT * FROM {table} where kind = {kind} ", Dict.create().set("table", tableName).set("kind", kind));
        RowMapper<T> rowMapper = new BeanPropertyRowMapper<>(clazz);
        log.debug("【执行SQL】SQL：{}", sql);
        log.debug("【执行SQL】参数：{}", JSONUtil.toJsonStr(kind));
